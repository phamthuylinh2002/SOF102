...
public class QLNguoiHoc extends javax.swing.JDialog {
    ...
    public boolean checkNull() {
        ...
        if (Hepler.checkNull(txtMaNH, "Mã người học không được trống")) {
            return false;
        }
        if (Hepler.checkNull(txtHoTen, "Họ tên người học không được trống")) {
            return false;
        }
        if (Hepler.checkNull(txtMatKhau, "Mật khẩu người học không được trống")) {
            return false;
        }
        if (Hepler.checkNull(txtXacNhanMK, "Xác nhận mật khẩu k được trống")) {
            return false;
        }
        if (Hepler.checkNull(txtNgaySinh, "ngày sinh không được trống")) {
            return false;
        }
        if (Hepler.checkDate(txtNgaySinh, "ngày sinh không đúng định dạng")) {
            return false;
        }
        if (Hepler.checkNull(txtDienThoai, "điện thoại không được trống")) {
            return false;
        }
        if (!Hepler.checkSDT(txtDienThoai)) {
            JOptionPane.showMessageDialog(this, "SDT không đúng định dạng");
            return false;
        }
        if (Hepler.checkNull(txtEmail, "email không được trống")) {
            return false;
        }
        if (!Hepler.checkMail(txtEmail.getText())) {
    ...
    private void initComponents() {
        ...
        jLabel9.setIcon(new javax.swing.ImageIcon(getClass().getResource("/Hinh/poly.png"))); // NOI18N
