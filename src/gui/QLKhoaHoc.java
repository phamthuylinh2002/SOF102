...
public class QLKhoaHoc extends javax.swing.JDialog {
    ...
    public boolean chechNull() {
        if (Hepler.checkNull(txtKhaiGiang, "ngày khai giảng k được trống")) {
            return false;
        }
        if (Hepler.checkDate(txtKhaiGiang, "ngày khai giảng phải là ngày")) {
            return false;
        }
        if (Hepler.checkNull(txtNguoiTao, "người tạo không được trống")) {
            return false;
        }
        return true;
    }
    ...
    public void delete() {
        if (!Auth.isManager()) {
            ...
            if (MsgBox.confirm(this, "Bạn chắc chắn muốn xoá khoá học này")) {
                try {
